    ...
    <div slot="oefening">
        ...
        <div v-for="(reeks,key) in oefenreeksen" :key="key" :style="'width:'+breedte+';'">
            <p
                ...
                :key="'r1_'+key">{{oef[0]}} <span v-if="toonOplossing" class="oplossing"><strong>{{oef[1]}}</strong></span></p>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      oefenreeks3: [],
      oefenreeksen:[],
      geselecteerdeTafels: [2, 3, 4, 5, 6, 7],
  ...
  methods: {
    ...
    uitrekenen() {
      ...
      for(var i=0;i<this.aantal/5;i++){
        var reeks = tafeloefening.getReeks(        this.opties.geselecteerdeBewerkingen,
        5
      )
