<template>
  <leeg-kader :oefenvormen="oefenvormen" :strookId="strookId">
    ...
    <div slot="oefening">
      <oefeningtitel :toonTitel="toonTitel">Hoe laat is het?</oefeningtitel>
      <div class="fit row justify-around">
        <div class="klokkaart" v-for="(oef, key) in oefeningen" :key="key">
          <klok
            ...
            class="q-ml-lg"
            :breedte="breedte"
            :toonWijzers="true || toonOplossing"
          ></klok>
          <div class="text-h6 text-center verwoording" v-if="toonOplossing">
            {{ oef.verwoording }}
          </div>
          <div v-else class="schrijflijn" :style="{ width: breedte }"></div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      //Inherent aan de oefening
      oefenopties:["aantal","verwoording","nauwkeurigheid","posities"]
    }
  },
